...
for(trial in 1:numTrials)
{
	...
	for(i in 1:sample_size)
	{
		...
		decision_tree = rpart(class~., data=training_set, method='class')
		prediction<-predict(decision_tree, newdata=row[-5], type='class')
		confMat<-table(row$class, prediction)
		correct_predictions<- correct_predictions + sum(diag(confMat))	
	}
...
xaxis<-c("Resubstitution", "Holdout", "10f-XValidation", "LOOCV")

plot(xaxis# Resubstitution
library(rpart)
...
barplot(combined, beside=TRUE, main="Iris")
